...
여러 트랜잭션이 동시에 동일한 데이터를 수정하지 못하도록 막는 방식이다.
...
- **개념**
  ...
  - JPA에서는 `@Lock(LockModeType.PESSIMISTIC_WRITE)` 를 사용한다.
- **동작 방식**
...
- **장점**
  - 충돌이 일어날 가능성이 높을 때 안전하다.
  - 실제 수정 시점에서의 데이터 정합성을 확실히 보장한다.
- **단점**
  - 데드락(Deadlock) 가능성이 존재한다.
  - 락 대기 시간으로 인해 성능이 저하될 수 있다.
- **적용 예시**
...
- **동작 방식**
  1. 데이터를 읽을 때 버전 정보를 함께 가져온다.
  2. 커밋 시점에 버전이 동일하면 업데이트하고, 다르면 `OptimisticLockException` 발생.
  3. 예외가 발생하면 애플리케이션에서 재시도 로직을 구현한다.
- **장점**
